...
interface ListaTopicosProps {
  ...
  calcularVotos: (topico: Topico) => { favoraveis: number, desfavoraveis: number };
}

const ListaTopicos: React.FC<ListaTopicosProps> = ({ topicos, onVoto, calcularVotos }) => {
  return (
    <div className="topicos-container">
      {topicos.map((topico) => (
        <div key={topico.id} className={`topico topico-item ${topico.active ? '' : 'inativo'}`}>
          ...
          <p>Votos Favoráveis: {calcularVotos(topico).favoraveis}</p>
          <p>Votos Desfavoráveis: {calcularVotos(topico).desfavoraveis}</p>
